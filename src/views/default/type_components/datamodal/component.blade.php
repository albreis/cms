<div class='{{$col_width?:'col-sm-10'}}form-datepicker {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}'
     style="{{@$form['style']}}">
    ...
    <div class="">
        ...
        <button class="btn btn-primary" onclick="showModal{{$name}}()" type="button"><i class='fa fa-search'></i> {{cbLang('datamodal_browse_data')}}</button>
                ...
                <a class="btn btn-info" href="{{CMS::adminPath()}}/{{$form['datamodal_module_path']}}" target="_blank"><i
...
@push('bottom')
    <script type="text/javascript">
        var url_{{$name}} = "{{CMS::mainpath('modal-data')}}?table={{$form['datamodal_table']}}&columns=id,{{$form['datamodal_columns']}}&name_column={{$name}}&where={{urlencode($form['datamodal_where'])}}&select_to={{ urlencode($form['datamodal_select_to']) }}&columns_name_alias={{ urlencode($form['datamodal_columns_alias']) }}&paginate={{urlencode($form['datamodal_paginate'])}}";
